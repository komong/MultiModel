...
### 2.1 模型配置（model-platform/config.yaml）
...
### 3.1 cost map 警告：getaddrinfo failed
...
### 3.3 PowerShell 中 curl 不支持 -H 参数
...
**解决**：推理模型 max_tokens 提升至 8192（见 2.3 节）。调整后 `finish_reason=stop`，content 正常输出。

### 3.5 test_call.py 端口不一致
...
### 4.5 litellm 版本过低导致 glm-5-1 不可用
